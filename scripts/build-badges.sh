...
# repo plus the GitHub release. Each reads the version actually recorded in that
# repo's freshly built metadata (not just the passed version), so a badge proves
# the repo really got that version; on any parse miss it falls back to <V>-<REL>.
#
...
#
# Usage: build-badges.sh <site-dir> <version> <pkgrel>
set -euo pipefail

SITE="${1:?usage: build-badges.sh <site> <version> <pkgrel>}"
V="${2:?version}"; REL="${3:?pkgrel}"
FALLBACK="${V}-${REL}"
COLOR="7c3aed"
BADGES="$SITE/badges"
mkdir -p "$BADGES"
...
echo "build-badges: wrote $BADGES (release=$FALLBACK dnf=${rpm_ver:-$FALLBACK} pacman=${arch_ver:-$FALLBACK} apt=${deb_ver:-$FALLBACK})"

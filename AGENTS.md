...
Guidance for working on **Codex-desktop-repack**. Read this before making changes.
...
It repackages Anthropic's **official** Codex Desktop Linux build (shipped only as
...
repo). Not affiliated with Anthropic. Source of truth: <https://Codex.com/download>.

### Names: package vs. app identity

Two names on purpose, do not conflate them:

- **Package name is `Codex-desktop-repack`** so `dnf`/`apt`/`rpm -q` show it as
  ours and it never collides with `Codex-desktop-bin` or a manual install. The
  package `Provides:`/`Conflicts:` the plain `Codex-desktop` so the two cannot
  coexist.
- **App identity stays `Codex-desktop`**: the binary, `/usr/bin/Codex-desktop`,
  the `.desktop` file, the icon, the WM_CLASS and the `Codex://` handler all keep
...
It then drops the `Codex-desktop-hotkey` helper next to `/usr/bin/Codex-desktop`,
and replaces that upstream symlink with `packaging/launcher/Codex-desktop-launcher`
...
packaging/
  rpm/Codex-desktop-repack.spec    the RPM spec (Release: %{_pkgrel}%{?dist})
  ...
  launcher/Codex-desktop-hotkey    Quick Entry hotkey helper (socket poker)
  launcher/Codex-desktop-launcher  installed as /usr/bin/Codex-desktop (rpm/deb/Arch);
...
- **`quick-entry-cli-toggle`** exposes the Quick Entry toggle over a Unix socket
  ...
  electron/electron#51875). The shipped `Codex-desktop-hotkey` command + a native
  GNOME shortcut drive it.
- **`quick-entry-app-id`** gives the Quick Entry window its own WM_CLASS
  (`Codex-quick-entry`) so corner/shadow extensions can exclude just it.
...
The build fails and `notify-failure` opens "Patch validation failed: Codex
...
```bash
node -e 'const a=require("@electron/asar"); require("fs").writeFileSync("/tmp/index.js", a.extractFile("PAYLOAD/usr/lib/Codex-desktop/resources/app.asar", ".vite/build/index.js"))'
```
...
- **A label-triggered Codex auto-fix routine handles failures.** When the
  release/CI workflow fails, `notify-failure` files an issue labeled
  `patch-failure` / `path-drift` / `ci-failure`, which triggers an automated Codex
